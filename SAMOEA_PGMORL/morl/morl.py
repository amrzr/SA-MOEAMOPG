...
from moea_surr import optimize
...
def run(args):
    ...
    nds_moea_objs = None
    nds_moea_policies = None
    nds_moea_env_params = None
    archive_all_offspring_batch = [[] for _ in range(len(scalarization_batch))]
    episode = 0
    iteration = 0
    while iteration < total_num_updates:
        ...
        while cnt_done_workers < len(processes):
            ...
            for sample in offsprings:
                ...
                archive_all_offspring_batch[task_id].append(Sample.copy_from(sample))
            if rl_results['done']:
                cnt_done_workers += 1
        ...
        nds_moea_objs, nds_moea_policies, nds_moea_env_params = optimize(archive_all_offspring_batch, 
                                                          ep, 
                                                          device, 
                                                          nds_moea_objs, 
                                                          nds_moea_policies, 
        ...
        # save moea solutions
        moea_dir = os.path.join(args.save_dir, str(iteration), 'moea')
        os.makedirs(moea_dir, exist_ok = True)
        with open(os.path.join(moea_dir, 'objs.txt'), 'w') as fp:
            for obj in nds_moea_objs:
                fp.write(('{:5f}' + (args.obj_num - 1) * ',{:5f}' + '\n').format(*obj))
        moea_data_iteration = {"nds_moea_policies":nds_moea_env_params,
                        "nds_moea_env_params":nds_moea_policies,
                        "nds_moea_objs":nds_moea_objs}
        with open(os.path.join(moea_dir,'moea_data_iteration.pickle'), 'wb') as handle:
            pickle.dump(moea_data_iteration, handle, protocol=pickle.HIGHEST_PROTOCOL)
        fig = plt.figure()
        plt.scatter(nds_moea_objs[:,0],nds_moea_objs[:,1],marker='*')
        plt.scatter(ep.obj_batch[:,0],ep.obj_batch[:,1])
        plt.savefig(os.path.join(moea_dir, 'plot.pdf'))
