...
class BaseDecompositionEA(BaseEA):
    ...
    def __init__(
        ...
        if initial_population is not None:
            ...
            if population_size is None and lattice_resolution is not None:
                population_size = comb(
                        lattice_resolution + num_fitnesses - 1,
                        num_fitnesses - 1,
                        exact=True,
                    )
                self.population.pop_size = population_size
    ...
    def _next_gen(self):
        ...
        #### re-evaluating parents
        if self.reevaluate_parents:
        ...
        #if True:
        #    n_best = pygmo.select_best_N_mo(self.non_dominated["individuals"], 10)
            #self.population.add(self.non_dominated["individuals"],self.use_surrogates)
        #    self.population.add(self.non_dominated["individuals"][n_best],self.use_surrogates)

        #if np.shape(self.population.individuals)[0]<=80:
        #    individuals_new = create_new_individuals(design="LHSDesign", problem=self.population.problem, pop_size=30)
            #offspring = np.vstack((offspring,individuals_new))
        #    self.population.add(individuals_new, self.use_surrogates)
        if not self.use_surrogates:
            #self._function_evaluation_count += offspring.shape[0]
            self._function_evaluation_count += self.population.objectives.shape[0]
        selected = self._select()
        #print("population:",self.population.objectives)
    ...
    def pre_iteration(self):
        ...
        if not self.interact:
            if self._iteration_counter > 1:
                ...
                print("Adapting!!!")
        if self._interaction_location != "Selection" and self._iteration_counter > 1:
            self.selection_operator.adapt_RVs(self.population.fitness)
